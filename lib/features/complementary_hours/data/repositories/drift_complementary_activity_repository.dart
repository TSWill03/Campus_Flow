...
class DriftComplementaryActivityRepository
    implements ComplementaryActivityRepository {
  DriftComplementaryActivityRepository(this._database, this._syncQueueService);
  ...
  @override
  Future<ComplementaryActivity?> findById(String id) async {
    final query = _database.select(_database.complementaryActivities)
      ..where((table) => table.id.equals(id))
      ..limit(1);
  ...
  @override
  Future<List<ComplementaryActivity>> getAllActivities({
  ...
  @override
  Future<void> saveActivity(ComplementaryActivity activity) async {
  ...
  @override
  Stream<List<ComplementaryActivity>> watchActivities({
    String? academicProfileId,
  }) {
    final query = _database.select(_database.complementaryActivities)
      ..where((table) => table.isDeleted.equals(false))
  ...
  ComplementaryActivity _mapRow(ComplementaryActivityRow row) {
